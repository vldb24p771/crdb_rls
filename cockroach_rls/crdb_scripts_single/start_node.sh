...
do
{
    ...
    ssh_command=" /root/cockroach/cockroach start --store=node$NUMBER --advertise-addr=$container_name:$PORT3 --http-addr=$container_name:$PORT1 --listen-addr=$container_name:$PORT3 --sql-addr=$container_name:$PORT2  --insecure --join=$join --background"
    echo $ssh_command
    echo $IP
