...
func (s *intervalSkl) AddRange(from, to []byte, opt rangeOptions, val cacheValue) {
	...
	//log.Infof(context.Background(), "AddRange, the from is %s and to is %s and opt is %d and val is %s", from, to, opt , val)
...
func (s *intervalSkl) addRange(from, to []byte, opt rangeOptions, val cacheValue) *sklPage {
	...
	if len(from) == 0 && len(to) == 0 {
		log.Infof(context.Background(), "addRange, the %s from is zero and to is zero", val)
	} else if len(to) == 0 {
		log.Infof(context.Background(), "addRange, the %s from is %s and to is zero", val, from)
	} else if len(from) == 0 {
		log.Infof(context.Background(), "addRange, the %s from is zero and to is %s", val, to)
	} else {
		log.Infof(context.Background(), "addRange, the %s from is %s and to is %s", val, from, to)
	}
	log.Infof(context.Background(), "addRange, the hasKey is %d and hasGap is %d opt, excludeFrom, excludeTo %d %d %d", hasKey, hasGap, opt, excludeFrom, excludeTo)
*/
	...
	it.Init(fp.list)
	//log.Infof(context.Background(), "addRange, the meta1 is %d", it.Meta())
	...
	if to != nil {
		...
		//log.Infof(context.Background(), "addRange, the meta1.5 is %d", it.Meta())
		if errors.Is(err, arenaskl.ErrArenaFull) {
			return fp
		}
	}
	//log.Infof(context.Background(), "addRange, the meta2 is %d", it.Meta())
	...
	//log.Infof(context.Background(), "addRange, the meta3 is %d", it.Meta())
	if errors.Is(err, arenaskl.ErrArenaFull) {
		return fp
	}
...
func (s *intervalSkl) rotatePages(filledPage *sklPage) {
	...
	//log.Infof(context.Background(), "rotatePages, the maxTime is %s", fp.getMaxTimestamp())
...
func (s *intervalSkl) LookupTimestampRange(from, to []byte, opt rangeOptions) cacheValue {
	...
	//log.Infof(context.Background(), "start!!!!!!!!LookupTimestampRange, the from is %s and to is %s", from, to)
	// Acquire the rotation mutex read lock so that the page will not be rotated
	// while add or lookup operations are in progress.
	s.rotMutex.RLock()
	defer s.rotMutex.RUnlock()
	...
	for e := s.pages.Front(); e != nil; e = e.Next() {
		...
		if !maxTS.Less(MaxTS.ts) {
			//log.Infof(context.Background(), "LookupTimestampRange, the MaxTS is %s", MaxTS)
			MaxTS.ts = maxTS
		}

		val2 := p.lookupTimestampRange(from, to, opt)
		val, _ = ratchetValue(val, val2)
		//log.Infof(context.Background(), "LookupTimestampRange, the MaxTS is %s the maxTS is %s and val is %s and val2 is %s", MaxTS, maxTS, val, val2)
	}
	...
	val.ts.WallTime = MaxTS.ts.WallTime
	//log.Infof(context.Background(), "LookupTimestampRange, the val is %s, the MaxTS is %s", val, MaxTS.ts)
	return val
}
...
func (p *sklPage) addNode(
	...
	//log.Infof(context.Background(), "addNode hasKey is %d hasGap is %d and opt is %d and it.Meta is %d", hasKey, hasGap, opt, it.Meta())
	if (opt & hasGap) != 0 {
	...
	if !it.SeekForPrev(key) {
		...
		if it.Valid() && bytes.Equal(it.Key(), key) {
			...
			//log.Infof(context.Background(), "addNode ratchetMaxTimestamp, maxtimestamp is %s", p.getMaxTimestamp())
			////log.Infof(context.Background(), "addNode, the meta1 is %d", it.Meta())
			...
			////log.Infof(context.Background(), "addNode, the meta2 is %d", it.Meta())
			err = it.Add(key, b, meta)
			////log.Infof(context.Background(), "addNode, the meta3 is %d", it.Meta())
		}

		//log.Infof(context.Background(), "addNode not found, the key is %s and val is %s and hasGap is %d", key, val, hasGap)
		switch {
	...
	//log.Infof(context.Background(), "addNode, the key is %s and val is %s and hasGap is %d", key, val, hasGap)
	//log.Infof(context.Background(), "addNode, the meta3 is %d", it.Meta())
...
func (p *sklPage) ensureFloorValue(it *arenaskl.Iterator, to []byte, val cacheValue) bool {
	for it.Valid() {
		...
		if (len(it.Key()) != 0) {
			//log.Infof(context.Background(), "ensureFloorValue, the key is %s and to is %s", it.Key(), to)
		} else {
			log.Infof(context.Background(), "ensureFloorValue, the key is zero and to is %s", to)
		}*/
...
func (p *sklPage) ratchetValueSet(
	...
	for {
		util.RacePreempt()
		//log.Infof(context.Background(), "ratchetValueSet, the meta1 is %d", it.Meta())
		meta := it.Meta()
		...
		////log.Infof(context.Background(), "ratchetValueSet, the meta2 is %d", newMeta)
		...
		////log.Infof(context.Background(), "ratchetValueSet, the meta2.5 is %d", it.Meta())
		if updateVals {
			...
			//log.Infof(context.Background(), "ratchetValueSet ratchetMaxTimestamp, maxtimestamp is %s", p.getMaxTimestamp())
			////log.Infof(context.Background(), "ratchetValueSet, the meta3 is %d", newMeta)
			// Remove the hasKey and hasGap flags from the meta. These will be
			// replaced below.
			newMeta &^= (hasKey | hasGap)
			////log.Infof(context.Background(), "ratchetValueSet, the meta4 is %d", newMeta)
			// Update the values, possibly preserving the init bit.
			b, valMeta := encodeValueSet(arr[:0], keyVal, gapVal)
			newMeta |= valMeta
			////log.Infof(context.Background(), "ratchetValueSet, the meta5 is %d", newMeta)
			err := it.Set(b, newMeta)
			////log.Infof(context.Background(), "ratchetValueSet, the meta6 is %d", newMeta)
			switch {
			case err == nil:
				// Success.
				return nil
			...
			case errors.Is(err, arenaskl.ErrArenaFull):
				...
				if !inited && (meta&cantInit) == 0 {
					...
					////log.Infof(context.Background(), "ratchetValueSet, the meta7 is %d", newMeta)
					switch {
					case errors.Is(err, arenaskl.ErrRecordUpdated):
			...
			////log.Infof(context.Background(), "ratchetValueSet, the meta2.8 is %d", it.Meta())
			switch {
			case err == nil:
				// Success.
				return nil
...
func (p *sklPage) maxInRange(it *arenaskl.Iterator, from, to []byte, opt rangeOptions) cacheValue {
	...
	//log.Infof(context.Background(), "maxInRange opt is %d and it.Meta is %d", opt, it.Meta())
	if !it.Valid() {
		// No more nodes.
		return prevGapVal
	...
	if len(from) == 0 && len(to) == 0 {
		//log.Infof(context.Background(), "maxInRange, the from is zero and to is zero")
	} else if len(to) == 0 {
		//log.Infof(context.Background(), "maxInRange, the from is %s and to is zero", from)
	} else if len(from) == 0 {
		log.Infof(context.Background(), "maxInRange, the from is zero and to is %s", to)
	} else {
		log.Infof(context.Background(), "maxInRange, the from is %s and to is %s", from, to)
	}*/
	...
	//log.Infof(context.Background(), "maxInRange, the maxval is %s", maxVal)
	return maxVal
}
...
func (p *sklPage) scanTo(
	...
	for {
		...
		if toCmp > 0 || (toCmp == 0 && (opt&excludeTo) != 0) {
			//log.Infof(context.Background(), "toCmp > 0, the key is %s, the maxVal is %s, to is %s, toCmp is %d", it.Key(), maxVal, to, toCmp)
		...
		//log.Infof(context.Background(), "scanTo, the key is %s, the maxVal is %s and the keyVal is %s and the gapVal is %s, to is %s, meta is %d", it.Key(), maxVal, keyVal, gapVal, to, it.Meta())
		...
		//log.Infof(context.Background(), "toCmp == 0, the meta1.5 is %d", it.Meta())
		...
		if toCmp == 0 {
			//log.Infof(context.Background(), "toCmp == 0, the meta2 is %d, the key is %s, the maxVal is %s, to is %s", it.Meta(), it.Key(), maxVal, to)
...
func decodeValueSet(b []byte, meta uint16) (keyVal, gapVal cacheValue) {
	...
	//log.Infof(context.Background(), "decodeValueSet, the meta is %d, the hasKey is %d, the hasGap is %d, the keyVal is %s and the gapVal is %s", meta, hasKey, hasGap, keyVal, gapVal)
	return
}

func encodeValueSet(b []byte, keyVal, gapVal cacheValue) (ret []byte, meta uint16) {
	//log.Infof(context.Background(), "encodeValueSet, the meta1 is %d", meta)
	if !keyVal.ts.IsEmpty() {
		b = encodeValue(b, keyVal)
		meta |= hasKey
	}
	//log.Infof(context.Background(), "encodeValueSet, the meta2 is %d", meta)
	if !gapVal.ts.IsEmpty() {
		b = encodeValue(b, gapVal)
		meta |= hasGap
	}
	//log.Infof(context.Background(), "encodeValueSet, the meta3 is %d", meta)
	ret = b
	return
}
