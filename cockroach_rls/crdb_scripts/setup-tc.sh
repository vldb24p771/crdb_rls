...
tc qdisc add dev eth0 parent 1:1 handle 22: netem delay 0ms
if [ $# -eq 1 ]
then 
  tc qdisc add dev eth0 parent 1:2 handle 11: netem delay 0ms
else 
  tc qdisc add dev eth0 parent 1:2 handle 11: netem delay 0ms
fi
tc filter add dev eth0 parent 1: prio 2 protocol ip u32 match ip dst ${my_ip}/32  classid 1:1
